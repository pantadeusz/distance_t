...
{
public:
    constexpr generic_position_t() : std::array<T, N>(){
        for (auto &e:*this) e=0;
    };
    constexpr generic_position_t(std::initializer_list<T> v) : std::array<T, N>()
    {
        std::size_t i = 0;
        for (auto &e:v) {
    ...
    constexpr generic_position_t(const std::vector<T> &v) : std::array<T, N>()
    {
    ...
    constexpr generic_position_t(const std::array<T, N-1> &v) : std::array<T, N>()
    {
        for (std::size_t i = 0; i < std:: min(v.size(),this->size()); i++) {
            this->operator[](i) = v[i];
        }
